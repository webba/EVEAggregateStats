...
for (const key in esijson.properties) {
    if (esijson.properties.hasOwnProperty(key) && key != "year") {
        ...
        temp += '\t<table class="table table-dark">';
...
for (const key in esijson.properties) {
    if (esijson.properties.hasOwnProperty(key) && key != "year") {
        const element = esijson.properties[key];
        //console.log(""+key);
        temp += '<div class=" row justify-content-md-center border-bottom pb-3 mb-4"><h1 clas="text-center" id="'+key+'" >'+key+'</h1></div><div *ngIf="stat.'+key+'" class="row justify-content-md-center">';
        temp += '<table class="table table-dark">';
        for (const key2 in element.properties) {
           temp += '<tr><td>'+key2+'</td><td><app-character-stat [value]="stat.'+key+'.'+key2+'"></app-character-stat></td></tr>'
        }
        //console.log(""+temp)
        temp += "</table></div>"
    }
